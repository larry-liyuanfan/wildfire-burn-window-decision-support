...
| D022 | Aggregate source-grid rule results with a normalized sparse burn-ID overlay | The verified contract contains only 351 non-zero intersections for 176 IDs; district-wide counts cannot be reassigned to units | Evaluate all 8/8 conditions at grid level, normalize overlap hectares per ID to `1±1e-6`, and forbid nearest fallback |
| D023 | Keep the area fraction continuous and treat 0.5/0.8/1.0 as sensitivity only | A hard cutoff would hide mixed-cell support and turn a reporting choice into the primary metric | Persist restricted hourly fractions; publish annual summaries and threshold-specific 2/4/6-hour maximal-segment counts |
| D024 | Define valid hours and limiting factors from complete 8/8 inputs | Missing proxy inputs must not silently become unsuitable weather or a causal explanation | Break threshold segments at invalid hours and rank limiting factors only over complete-validity weighted cell-hours |
| D025 | Expose climatology through an allowlisted precomputed-artifact tool | Letting a caller supply paths or executable array expressions would cross the service trust boundary | `get_burn_unit_climatology` filters a startup-validated, hash-pinned compact artifact and performs no recomputation |
