...
| Chunked public weather-screen checkpoint/restart | `verified-local-contract` | 49 local tests; streaming accumulator matches batch summaries across month/chunk boundaries, restores unfinished per-cell runs, and comparator rejects semantic changes | Code contract only until the 336-hour controlled-stop/resume Spartan gate completes; still a weather-only upper bound |
